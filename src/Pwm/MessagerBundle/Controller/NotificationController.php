...
class NotificationController extends Controller
{
  ...
  */
  ...
  public function sendSMSAction(Request $request)
    ...
    $form->handleRequest($request);
    if ($form->isSubmitted() && $form->isValid()) {
    $path = $this->get('kernel')->getRootDir(). "/../web/import/contacts-dbs-samedi.xlsx";
   ...
   $logPath = $this->get('kernel')->getRootDir(). "/../web/contacts-dbs-samedi.txt";
...
for ($row = 0; $row <= $highestRow; ++$row) {
       ...
       $url='https://api-public.mtarget.fr/api-sms.json?username=omegatelecombuilding&password=79sawbfF&msisdn='.$contacts.'&sender=ATLENTIQBANK&msg='.$msg;  
